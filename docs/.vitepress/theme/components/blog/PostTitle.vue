...
<template>
  <div class="mb-6">
    ...
    <h3
      class="mb-2 mt-2 text-2xl font-bold tracking-tight text-[color:var(--vp-c-brand-light)] dark:text-[color:var(--vp-c-brand-dark)]"
    >
    ...
    <div class="flex justify-between items-center mt-2 text-gray-500">
      <a
        v-if="prevPost"
        :href="`${site.base}blog${prevPost.href}`"
      ...
      <a
        v-if="nextPost"
        :href="`${site.base}blog${nextPost.href}`"
        class="inline-flex items-center font-medium dark:text-white hover:text-[color:var(--vp-c-brand-dark)]"
      >
        <span>Next Post</span>
